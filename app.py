...
@st.cache_resource
def carregar_modelo():
    if not os.path.exists('modelo/XGBRegressor.joblib'):
        st.error(
            "Modelo não encontrado. Certifique-se de que o arquivo 'XGBRegressor.joblib' está na pasta 'modelo'.")
        return None
    return joblib.load('modelo/XGBRegressor.joblib')
...
x_listas = {
    'property_type': ['Apartment', 'House', 'Condominium', 'Outro'],
